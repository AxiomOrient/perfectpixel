...
| --- | --- | --- | --- | --- | --- | --- |
| C1 | Raster inspection — `perfectpixel inspect` | Decode limits and immutable `RasterInspection` | `Result<RasterInspection, PpError>` | `ImageCodec` reads; stdout publishes JSON | Invalid input/limit returns JSON error; no file mutation | `src/io/codec.rs`, `tests/inspect_contract.rs`, `tests/codec_limits.rs` |
