...
const INSPECT_SCHEMA: &str = "perfectpixel.inspect/1";
...
fn chroma_plan(args: &[String]) -> PpResult<String> {
    ...
    let plan = perfectpixel::plan_chroma(&request.subject_rgb_colors)?;
...
fn edit(args: &[String]) -> PpResult<String> {
    ...
    // The encoder bytes are committed only after all request, input, and pure
    // transform validation has succeeded. AtomicFileWriter is the sole file
    // publication point for this operation.
    ...
    serialize_json(
        &EditSummary {
            ...
            output_sha256: perfectpixel::sha256_hex(&encoded),
...
impl EditStep {
    fn to_raster_edit(&self) -> PpResult<perfectpixel::RasterEdit> {
        Ok(match self {
            Self::Crop {
                x,
                y,
                width,
                height,
            } => perfectpixel::RasterEdit::Crop {
                x: *x,
                y: *y,
                width: *width,
                height: *height,
            },
            Self::Rotate { quarter_turns } => perfectpixel::RasterEdit::RotateQuarterTurns {
                quarter_turns: *quarter_turns,
            },
            Self::Flip { axis } => match axis {
                EditFlipAxis::Horizontal => perfectpixel::RasterEdit::FlipHorizontal,
                EditFlipAxis::Vertical => perfectpixel::RasterEdit::FlipVertical,
            },
            Self::Resize {
                width,
                height,
                filter,
            } => perfectpixel::RasterEdit::Resize {
                width: *width,
                height: *height,
                filter: match filter {
                    EditFilter::Nearest => perfectpixel::ResampleFilter::Nearest,
                    EditFilter::Lanczos3 => perfectpixel::ResampleFilter::Lanczos3,
                },
            },
            Self::RemoveBackground {
                ...
                tolerance,
                feather,
            } => perfectpixel::RasterEdit::RemoveBackground {
                keys: keys.clone(),
                tolerance: *tolerance,
                feather: *feather,
            },
            Self::RemoveBackgroundAuto {
                ...
                tolerance,
                feather,
            } => perfectpixel::RasterEdit::RemoveBackgroundAuto {
                max_keys: *max_keys,
